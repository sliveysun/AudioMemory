...
#llm = ChatOpenAI(model='hunyuan-pro')
#embeddings = OpenAIEmbeddings(model="text-embedding-3-large")
#parser = PydanticOutputParser(pydantic_object=Structured)
#llm_with_parser = llm.with_structured_output(Structured)


def create_langchain_components(api_key, base_url=None):
    """Creates and returns Langchain components with optional custom API key and base URL."""
    ...
    # OpenAIEmbeddings configuration
    embedding_kwargs = {"openai_api_key": api_key}
    if base_url:
        embedding_kwargs["openai_api_base"] = base_url
    embeddings = OpenAIEmbeddings(model="text-embedding-3-large", **embedding_kwargs)

    # Output parser (remains the same)
    parser = PydanticOutputParser(pydantic_object=Structured)
    llm_with_parser = llm.with_structured_output(Structured)

    return llm, embeddings, parser, llm_with_parser


api_key: str = os.environ.get("TENCENT_HUNYUAN_API_KEY")
base_url = 'https://api.hunyuan.cloud.tencent.com/v1'
llm, embeddings, parser, llm_with_parser = create_langchain_components(api_key, base_url)
